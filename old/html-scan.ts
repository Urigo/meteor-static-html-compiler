...
export function scan(options): Tag[] {
 ...
 */
export class HtmlScan {
  public parseError: Function;
  ...
  constructor({
    ...
    let nonMatchingTagsFound: boolean = false;
    ...
    while (this.rest) {
      ...
      if (matchTokenUnsupported) {
        switch (matchTokenUnsupported.toLowerCase()) {
          ...
          break;
          default: this.throwCompileError();
        }
      }
    ...
    if (!foundMatch) {
      ...
      console.log(`
        WARNING: ${this.sourceName}
        A tag that is not of ${tagNames.join(', ')} was found in a file
        that contains at least one of those,
        and will be ignored
      `);
    }
  }
  ...
  private throwCompileError(msg?: string, overrideIndex?: number) {
